...
Phase 1-3, P0-P1, Phase 5 全部完成。Phase 4 () 为外部仓库，待后续。
...
| 1.3 | 搬迁 `tools/cache/` → `go-common/cache/`，底层替换为 `github.com/samber/hot`（删除 `core/` 下自定义 FIFO/LRU/LFU/CLOCK/MRU 实现，对齐 的 `hot.NewHotCache[K,V](hot.LRU, max).Build()` 用法） | 1.5h |
...
| 2.5 | **新建** `go-middleware/es/` — Elasticsearch 客户端 + 配置（反哺） | 2h |
| 2.6 | **新建** `go-middleware/clickhouse/` — ClickHouse 客户端 + 配置（反哺） | 2h |
...
| 3.6 | **新增** `go-framework/kitex/middleware/` — 从  interceptor.yaml 回流中间件 | 3h |
...
- rpcerror 统一用 `oops` 风格（ 方案），但保留 go-tools 原有 `ErrorType` 枚举作为辅助
...
### 目标
从"内嵌实现"改为"import go-tools 三库 + 薄适配层"。

### 任务清单

| # | 任务 | 工作量 |
|---|------|--------|
| 4.1 | **第一步**：添加 go-tools 三库到  的 `go.mod`（开发依赖） | 0.5h |
...
| 5.3 | 更新  文档（README.md / README.zh-CN.md / docs/examples.md） | 2h |
| 5.4 | 更新  生成的项目中嵌入的设计文档（`internal/assets/_data/docs/`） | 2h |
...
| ES/ClickHouse 为全新中间件，反哺经验有限 | 中 | 中 | 参考  现有 optional/clickhouse.go 和 optional/es.go 模板，优先保证基本连接能力 |
| go-tools 测试不全，搬迁后暴露 bug | 中 | 中 | Phase 1-3 每步都跑测试，提前发现 |
|  Golden 测试更新量过大 | 高 | 中 | 分批更新 golden fixture，每次只更新一个模板对应的测试 |
...
| # | 决策 | 影响范围 | 结论 | 状态 |
|---|------|---------|------|------|
| D1 | Kafka 库：kafka-go vs sarama？ | Phase 2, 4 | **kafka-go**（与  已选型一致），sarama 标记 deprecated | ✅ 已确认 |
...
| Phase 4 () | revert  的模板改动 commit，Golden 测试还原 | < 10 分钟 |
