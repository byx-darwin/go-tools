...
go-middleware  20000-20699（码段边界，包内按需定义）
  redis        20101      → HTTP 503（连接/Ping 失败）
  kafka        20201-20203 → HTTP 500（write/read/commit 失败）
  db           20301-20302 → HTTP 500/503（open 失败 20301 → 500，connect 失败 20302 → 503）
  clickhouse   20401-20403  → HTTP 503（query 失败 20402 → 500）
  tls          20501-20504  → HTTP 503（send 失败 20502 → 500）
  es           20701       → HTTP 503（客户端初始化失败）
...
详见 `go-common/error/`（机制）、`go-framework/error/`（框架码）、`go-auth/error/`（认证码）、`go-middleware/{redis,kafka,db,clickhouse,tls,es}/errors.go`（中间件码）。
