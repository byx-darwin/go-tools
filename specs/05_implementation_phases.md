...
| 1.3 | 搬迁 `tools/cache/` → `go-common/cache/`，底层替换为 `github.com/samber/hot`（删除 `core/` 下自定义 FIFO/LRU/LFU/CLOCK/MRU 实现，对齐 ncgo 模板的 `hot.NewHotCache[K,V](hot.LRU, max).Build()` 用法） | 1.5h |
...
| 1.10 | **新建** `go-common/log/` — 基于 `log/slog` 的日志库（slog.Handler + lumberjack 轮转 + gzip 压缩 + OTel span 联动）+ `adapters/` 包（klog/hlog 适配器，对齐 ncgo 模板） | 3h |
...
| 2.5 | **新建** `go-middleware/es/` — Elasticsearch 客户端 + 配置（ncgo 模板反哺） | 2h |
| 2.6 | **新建** `go-middleware/clickhouse/` — ClickHouse 客户端 + 配置（ncgo 模板反哺） | 2h |
...
| 3.6 | **新增** `go-framework/kitex/middleware/` — 从 ncgo interceptor.yaml 回流中间件 | 3h |
...
**关键决策**：
- 中间件是从 ncgo 模板**反哺**到 go-framework（模板经验 → 库代码）
- rpcerror 统一用 `oops` 风格（ncgo 方案），但保留 go-tools 原有 `ErrorType` 枚举作为辅助
...
### 目标
ncgo 模板从"内嵌实现"改为"import go-tools 三库 + 薄适配层"。

### 任务清单

| # | 任务 | 工作量 |
|---|------|--------|
| 4.1 | **第一步**：添加 go-tools 三库到 ncgo 的 `go.mod`（开发依赖） | 0.5h |
| 4.2 | 改造 `kitex/kitex-template/conf.yaml` — 引入 go-framework config | 2h |
| 4.3 | 改造 `kitex/kitex-template/interceptor.yaml` — re-export go-framework 中间件 | 1h |
| 4.4 | 改造 `kitex/kitex-template/rpcerror.yaml` — re-export go-framework rpcerror | 1h |
| 4.5 | 改造 `kitex/kitex-template/server.yaml` — 使用 go-framework option 工厂 | 2h |
| 4.6 | 改造 `hertz/layout.yaml` 中的 response 包 — 引入 go-framework response | 1h |
| 4.7 | 改造 `hertz/optional/redis.go` — 引入 go-middleware redis | 1h |
| 4.8 | 改造 `hertz/optional/kafka.go` — 引入 go-middleware kafka | 1h |
| 4.9 | 改造 `kitex/optional/redis.go` 等对应文件 | 1h |
| 4.10 | 改造 `kitex/optional/observability_logging.go` + `hertz/optional/observability_logging.go` — 引入 go-framework 观测性 + go-middleware TLS | 1.5h |
| 4.11 | 更新 `go.mod` 模板 — 确保生成的项目自动依赖 go-tools 三库 | 0.5h |
| 4.12 | **更新 Golden 测试**：`go test ./internal/scaffold/mono/... -update-golden -count=1` | 1h |
| 4.13 | 全量测试：`go build ./... && go vet ./... && go test ./... -count=1` | 1h |

**风险**：
- Golden 测试的 fixture 大量变化，需要仔细 review
...
| # | 任务 | 工作量 |
|---|------|--------|
| 5.0 | **自动化冒烟测试**：CI 新增 job，每次 PR 自动执行：`ncgo generate` → 生成 Hertz + Kitex 项目 → `go build ./...` → `go vet ./...` | 2h |
| 5.1 | 端到端验证：生成 Hertz + Kitex 两个完整项目，确认可编译、可启动 | 3h |
| 5.2 | 更新 go-tools 各库的 README（go-common / go-middleware / go-framework） | 2h |
| 5.3 | 更新 ncgo 文档（README.md / README.zh-CN.md / docs/examples.md） | 2h |
| 5.4 | 更新 ncgo 生成的项目中嵌入的设计文档（`internal/assets/_data/docs/`） | 2h |
...
Phase 2 (go-middleware) ──→ 需要 go-common
    ↓
Phase 3 (go-framework)  ──→ 需要 go-common + go-middleware
    ↓
Phase 4 (ncgo 模板改造) ──→ 需要 Phase 1-3 全部完成
    ↓
Phase 5 (验证 + 文档)
```

Phase 1-3 **不可并行**（有依赖关系）。
Phase 4 可以与 Phase 3 部分重叠（先改不依赖新中间件的模板）。
...
| ES/ClickHouse 为全新中间件，ncgo 模板反哺经验有限 | 中 | 中 | 参考 ncgo 现有 optional/clickhouse.go 和 optional/es.go 模板，优先保证基本连接能力 |
| go-tools 测试不全，搬迁后暴露 bug | 中 | 中 | Phase 1-3 每步都跑测试，提前发现 |
| ncgo Golden 测试更新量过大 | 高 | 中 | 分批更新 golden fixture，每次只更新一个模板对应的测试 |
| 已有用户的项目 import 路径硬编码 | 低 | 低 | 旧路径保留 deprecated alias，给迁移时间 |
| oops 与 pkg/errors 迁移矛盾 | 低 | 中 | 已确认 D3，go-framework 提供转换函数 `FromPkgErrors` |

## 八、已确认关键决策

| # | 决策 | 影响范围 | 结论 | 状态 |
|---|------|---------|------|------|
| D1 | Kafka 库：kafka-go vs sarama？ | Phase 2, 4 | **kafka-go**（与 ncgo 已选型一致），sarama 标记 deprecated | ✅ 已确认 |
| D2 | 配置时间单位：秒 vs 毫秒 vs time.Duration？ | Phase 3, 4 | **time.Duration**（YAML 写 `30s`） | ✅ 已确认 |
| D3 | 错误库：仅 oops 还是同时保留 pkg/errors？ | Phase 3 | **oops 为主**，go-framework 提供 pkg/errors → oops 转换桥 | ✅ 已确认 |
| D4 | go-tools 三库发布策略：独立发版 vs workspace 联合？ | 全部 | **独立发版**（go-common 变化最少，go-framework 变化最多） | ✅ 已确认 |
| D5 | 旧 go-tools 模块（config/hertz/kitex/middleware/tools）处理？ | 全部 | **标记 deprecated**，go.work 中保留 1-2 版本周期 | ✅ 已确认 |
...
| Phase 4 (ncgo 模板) | revert ncgo 的模板改动 commit，Golden 测试还原 | < 10 分钟 |
